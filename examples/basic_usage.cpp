...
public:
    ...
    void operator()() const {
        std::cout << "Param handler called with no parameters" << std::endl;
    }
...
public:
    ...
    void operator()() const {
        std::cout << "Full handler called with no parameters" << std::endl;
    }
...
private:
    std::function<void(const std::map<std::string, std::string>&, 
                      const std::map<std::string, std::string>&)> func_;
};
...
void demonstrate_static_routes() {
    ...
    router_.add_route(HttpMethod::GET, "/about", []() {
        std::cout << "About page\n";
    });
    
    router_.add_route(HttpMethod::GET, "/contact", []() {
        std::cout << "Contact page\n";
    });
...
void demonstrate_parameterized_routes() {
    ...
    router_.add_route(HttpMethod::GET, "/users/:id/posts/:post_id", 
        ParamHandler([](const auto& params) {
            std::cout << "Post " << params.at("post_id") 
                      << " by user " << params.at("id") << "\n";
        }));
...
void demonstrate_wildcard_routes() {
    ...
    // Add wildcard routes
    router_.add_route(HttpMethod::GET, "/static/*", 
        ParamHandler([](const auto& params) {
            std::cout << "Serving static file: " << params.at("*") << "\n";
        }));
    
    router_.add_route(HttpMethod::GET, "/files/:type/*", 
        ParamHandler([](const auto& params) {
...
void demonstrate_query_parameters() {
    ...
    std::vector<std::string> test_paths = {
        "/search?q=router&sort=name&limit=10",
