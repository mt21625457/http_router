...
void benchmark_cache_effectiveness() {
    ...
    for (int i = 0; i < num_tests; ++i) {
        if (i < num_tests * 0.7) {
            ...
            test_paths.push_back("/api/resource" + std::to_string(route_id) + "/item" + std::to_string(i % 10));
        } else {
            // Random paths for cache misses
            int route_id = dis(gen);
            test_paths.push_back("/api/resource" + std::to_string(route_id) + "/item" + std::to_string(i));
        }
    }
    ...
    std::cout << "  Throughput: " << (test_paths.size() * 1000000LL / total_time.count()) << " lookups/sec\n";
}
...
void benchmark_memory_usage() {
    ...
    auto measure_routes = [](int num_routes) {
        ...
        std::string test_path = "/api/v1/resource" + std::to_string(num_routes / 2) + "/123/action/update";
        std::map<std::string, std::string> params, query_params;
        
        start_time = std::chrono::high_resolution_clock::now();
        auto result = router_.find_route(HttpMethod::GET, test_path, params, query_params);
        end_time = std::chrono::high_resolution_clock::now();
        auto lookup_time = std::chrono::duration_cast<std::chrono::nanoseconds>(end_time - start_time);
...
void benchmark_concurrent_access() {
    ...
    for (int t = 0; t < num_threads; ++t) {
        threads.emplace_back([&router_, t, lookups_per_thread, &total_operations, &successful_operations]() {
            std::random_device rd;
            std::mt19937 gen(rd());
            ...
            for (int i = 0; i < lookups_per_thread; ++i) {
                int route_id = dis(gen);
                std::string path = "/concurrent/resource" + std::to_string(route_id) + "/item" + std::to_string(i);
    ...
    std::cout << "  Success rate: " << (100.0 * successful_operations.load() / total_operations.load()) << "%\n";
    std::cout << "  Total time: " << total_time.count() << " ms\n";
    std::cout << "  Throughput: " << (total_operations.load() * 1000LL / total_time.count()) << " ops/sec\n";
    std::cout << "  Average per operation: " << (total_time.count() * 1000.0 / total_operations.load()) << " μs\n";
}
...
void benchmark_query_parameter_parsing() {
    ...
    std::vector<std::string> test_urls = {
        "/search?q=simple",
        "/search?q=hello&sort=name&order=asc",
        ...
        "/search?q=url%20encoded%20string&category=electronics&price_min=100&price_max=500&brand=apple&brand=samsung"
    };
...
void benchmark_large_scale_routing() {
    ...
    for (int i = 0; i < num_routes; ++i) {
        if (i % 4 == 0) {
            ...
            router_.add_route(HttpMethod::GET, "/users/" + std::to_string(i) + "/:id", FastHandler{});
        } else if (i % 4 == 2) {
            // Multiple parameter routes
            router_.add_route(HttpMethod::GET, "/api/v" + std::to_string(i % 5) + "/res" + std::to_string(i) + "/:id/sub/:sub_id", FastHandler{});
        } else {
            // Wildcard routes
    ...
    for (int i = 0; i < num_lookups; ++i) {
        ...
        if (route_id % 4 == 0) {
            ...
            path = "/api/v" + std::to_string(route_id % 5) + "/res" + std::to_string(route_id) + "/456/sub/789";
        } else {
    ...
    std::cout << "  Average per lookup: " << (lookup_time.count() * 1000.0 / num_lookups) << " μs\n";
    std::cout << "  Throughput: " << (num_lookups * 1000LL / lookup_time.count()) << " lookups/sec\n";
}
