...
public:
    ...
    using MiddlewareFunc =
        std::function<void(HandlerPtr &)>; ///< Middleware function type / 中间件函数类型
    using RouterRef = router<Handler> &;   ///< Reference to router / 路由器引用
    using GroupPtr = std::shared_ptr<router_group<Handler>>;   ///< Shared pointer to router
                                                               ///< group / 路由组的共享指针
    using WeakGroupPtr = std::weak_ptr<router_group<Handler>>; ///< Weak pointer to router group /
                                                               ///< 路由组的弱指针

private:
    RouterRef router_;                        ///< Reference to the main router / 主路由器的引用
    std::string prefix_;                      ///< Path prefix for this group / 此组的路径前缀
    std::vector<MiddlewareFunc> middlewares_; ///< Middleware stack for this group / 此组的中间件栈
...
public:
    ...
    GroupPtr group(std::string_view relative_prefix)
    {
        ...
        // 创建以此组为父组的嵌套组
        GroupPtr nested_group = create_group(router_, full_prefix, this->weak_from_this());
    ...
    std::vector<MiddlewareFunc> get_all_middlewares() const
    {
        ...
        // 添加此组的中间件
        all_middlewares.insert(all_middlewares.end(), middlewares_.begin(), middlewares_.end());
...
create_logger(std::function<void(const std::string &)> logger_func)
{
    return [logger_func](std::shared_ptr<Handler> &handler) {
        ...
        handler = std::make_shared<Handler>([original_handler, logger_func](auto... args) {
...
typename router_group<Handler>::MiddlewareFunc create_auth(std::function<bool()> auth_func)
{
    return [auth_func](std::shared_ptr<Handler> &handler) {
        auto original_handler = handler;
        handler = std::make_shared<Handler>([original_handler, auth_func](auto... args) {
            if (!auth_func()) {
...
create_cors(const std::vector<std::string> &allowed_origins = {"*"})
{
    return [allowed_origins](std::shared_ptr<Handler> &handler) {
        auto original_handler = handler;
        handler = std::make_shared<Handler>([original_handler, allowed_origins](auto... args) {
