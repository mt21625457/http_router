...
private:
    ...
    void CreateParameterizedLambdaRoutes(size_t& route_count, std::mt19937& gen)
    {
        ...
        std::vector<RoutePattern> patterns = {
            ...
            {"/api/users/:id/posts/:post_id", [](size_t i) { 
                return "/api/users/" + std::to_string(100 + i) + "/posts/" + std::to_string(200 + i); 
            }},
            {"/api/companies/:company_id/employees/:employee_id", [](size_t i) { 
                return "/api/companies/" + std::to_string(300 + i) + "/employees/" + std::to_string(400 + i); 
            }},
            {"/api/projects/:project_id/tasks/:task_id", [](size_t i) { 
                return "/api/projects/" + std::to_string(500 + i) + "/tasks/" + std::to_string(600 + i); 
            }},
            {"/api/orders/:order_id/items/:item_id", [](size_t i) { 
                return "/api/orders/" + std::to_string(700 + i) + "/items/" + std::to_string(800 + i); 
            }},
            {"/api/customers/:customer_id/addresses/:address_id", [](size_t i) { 
                return "/api/customers/" + std::to_string(900 + i) + "/addresses/" + std::to_string(1000 + i); 
            }},
            {"/api/products/:product_id/variants/:variant_id", [](size_t i) { 
                return "/api/products/" + std::to_string(1100 + i) + "/variants/" + std::to_string(1200 + i); 
            }},
            {"/api/categories/:category_id/subcategories/:subcategory_id", [](size_t i) { 
                return "/api/categories/" + std::to_string(1300 + i) + "/subcategories/" + std::to_string(1400 + i); 
            }},
            {"/api/reports/:report_id/sections/:section_id", [](size_t i) { 
                return "/api/reports/" + std::to_string(1500 + i) + "/sections/" + std::to_string(1600 + i); 
            }},
            {"/api/workflows/:workflow_id/steps/:step_id", [](size_t i) { 
                return "/api/workflows/" + std::to_string(1700 + i) + "/steps/" + std::to_string(1800 + i); 
            }}
        };
    ...
    void CreateWildcardLambdaRoutes(size_t& route_count, std::mt19937& gen)
    {
        ...
        for (size_t i = 0; i < TOTAL_ROUTES * 0.2 && route_count < TOTAL_ROUTES; ++i) {
            ...
            // 创建匹配通配符的测试路径
            std::string test_path = base + std::to_string(i % 1000) + "/subfolder/file" + std::to_string(i) + ".dat";
