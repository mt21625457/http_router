...
public:
     ...
     */
    std::optional<std::reference_wrapper<HandlerWrapper>> find_route(
        HttpMethod method, 
...
TEST_F(LambdaContextTest, ParameterExtractionLambda)
{
    ...
    auto result = router.find_route(HttpMethod::GET, "/users/123?format=json", params_, query_params_);
    EXPECT_TRUE(result.has_value());
...
TEST_F(LambdaContextTest, ComplexBusinessLogicLambda)
{
    ...
    auto user_handler = [&user_db](HttpContext* ctx) {
        ...
        if (it != user_db.end()) {
            ctx->set_response(200, "{\"id\": \"" + user_id + "\", \"name\": \"" + it->second + "\"}");
...
TEST_F(LambdaContextTest, LambdaPerformanceTest)
{
    ...
    for (int i = 0; i < iterations; ++i) {
        ...
        auto result = router.find_route(HttpMethod::GET, "/perf/" + std::to_string(route_id), params_, query_params_);
...
TEST_F(LambdaContextTest, LambdaVsRegularHandlerComparison)
{
    ...
    for (int i = 0; i < num_routes; ++i) {
        ...
        // 普通处理器路由
        regular_router.add_route(HttpMethod::GET, "/regular/" + std::to_string(i), RegularHandler{});
    }
    ...
    for (int i = 0; i < iterations; ++i) {
        int route_id = i % num_routes;
        auto result = lambda_router.find_route(HttpMethod::GET, "/lambda/" + std::to_string(route_id), params_, query_params_);
        if (result.has_value()) {
            result->get()();
        }
    }
    ...
    for (int i = 0; i < iterations; ++i) {
        int route_id = i % num_routes;
        auto result = regular_router.find_route(HttpMethod::GET, "/regular/" + std::to_string(route_id), params_, query_params_);
        if (result.has_value()) {
            result->get()();
        }
    }
    ...
    for (int t = 0; t < num_threads; ++t) {
        threads.emplace_back([&router, &success_count, operations_per_thread]() {
            for (int i = 0; i < operations_per_thread; ++i) {
                ...
                auto result = router.find_route(HttpMethod::GET, "/concurrent", local_params, local_query_params);
    ...
    // 创建大量lambda处理器来测试内存管理
    {
        ...
        for (int i = 0; i < 100; ++i) {
            auto handler = [large_data, i](HttpContext* ctx) {
                ctx->set_response(200, "Handler " + std::to_string(i) + " with data size: " + std::to_string(large_data.size()));
            };
