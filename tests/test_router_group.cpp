...
#include "falcon/router/router_group.hpp"
#include "falcon/router/router_impl.hpp"
...
private:
  ...
  void test_nested_groups() {
    ...
    int result = r.find_route(HttpMethod::GET, "/api/v1/users", found_handler,
                              params, query_params);
  ...
  void test_middleware_application() {
    ...
    api_group->use([](std::shared_ptr<TestHandler> &handler) {
      ...
      handler = std::make_shared<TestHandler>(original_handler->name +
                                              "_with_middleware");
    });
    ...
    int result = r.find_route(HttpMethod::GET, "/api/test", found_handler,
                              params, query_params);
  ...
  void test_multiple_nested_groups() {
    ...
    int result = r.find_route(HttpMethod::GET, "/api/v1/users/123",
  ...
  void test_group_inheritance() {
    ...
    int result = r.find_route(HttpMethod::GET, "/api/v1/test", found_handler,
                              params, query_params);
  ...
  void test_all_http_methods() {
    ...
    api_group->delete_("/test",
                       std::make_shared<TestHandler>("delete_handler"));
    ...
    api_group->options("/test",
                       std::make_shared<TestHandler>("options_handler"));
    ...
    for (const auto &[method, expected_name] : test_cases) {
      ...
      int result = r.find_route(method, "/api/test", found_handler, params,
                                query_params);
  ...
  void test_any_method() {
    ...
    std::vector<HttpMethod> methods = {HttpMethod::GET,    HttpMethod::POST,
    ...
    for (auto method : methods) {
      ...
      int result =
          r.find_route(method, "/api/any", found_handler, params, query_params);
  ...
  void test_parameter_extraction() {
    ...
    api_group->get("/files/:filename",
                   std::make_shared<TestHandler>("file_handler"));
    ...
    std::vector<std::tuple<std::string, std::map<std::string, std::string>>>
        test_cases = {
            ...
            {"/api/posts/100/comments/200",
             {{"id", "100"}, {"commentId", "200"}}},
  ...
  void test_error_handling() {
    ...
    // Non-existent route
    int result = r.find_route(HttpMethod::GET, "/api/nonexistent",
                              found_handler, params, query_params);
    assert(result == -1);

    // Wrong method
    api_group->get("/users", std::make_shared<TestHandler>("get_only"));
    result = r.find_route(HttpMethod::POST, "/api/users", found_handler, params,
                          query_params);
    assert(result == -1);
  ...
  void test_edge_cases() {
    ...
    // Test empty prefix
    int result = r.find_route(HttpMethod::GET, "/test", found_handler, params,
                              query_params);
    assert(result == 0);
    assert(found_handler->name == "empty_prefix");

    // Test root prefix
    result = r.find_route(HttpMethod::GET, "/root", found_handler, params,
                          query_params);
    assert(result == 0);
    ...
    result = r.find_route(HttpMethod::GET,
                          "/very/deep/nested/group/structure/endpoint",
                          found_handler, params, query_params);
    assert(result == 0);
    assert(found_handler->name == "deep_handler");
  ...
  void test_url_encoding() {
    ...
    int result = r.find_route(HttpMethod::GET,
                              "/api/search?q=hello%20world&filter=type%3Duser",
                              found_handler, params, query_params);
  ...
  void test_performance_large_groups() {
    ...
    for (int i = 0; i < num_routes; ++i) {
      ...
      api_group->get(
          path, std::make_shared<TestHandler>("handler_" + std::to_string(i)));
    }
    ...
    std::cout << "  Average lookup time: " << lookup_time / 100.0
  ...
  void test_performance_deep_nesting() {
    ...
    groups.back()->get("/endpoint",
                       std::make_shared<TestHandler>("deep_handler"));
    ...
    for (int i = 0; i < 100; ++i) {
      int result = r.find_route(HttpMethod::GET, deep_path, found_handler,
                                params, query_params);
      assert(result == 0);
    }
    ...
    std::cout << "  Deep nesting (" << depth
              << " levels) creation time: " << creation_time << "ms"
              << std::endl;
    std::cout << "  Average deep lookup time: " << lookup_time / 100.0
              << "ms per lookup" << std::endl;
  ...
  void test_performance_many_middlewares() {
    ...
    for (int i = 0; i < num_middlewares; ++i) {
      api_group->use([i](std::shared_ptr<TestHandler> &handler) {
        auto original = handler;
        handler = std::make_shared<TestHandler>(original->name + "_mw" +
                                                std::to_string(i));
      });
    }
    ...
    std::cout << "  Average lookup with middlewares: " << lookup_time / 100.0
              << "ms per lookup" << std::endl;
  ...
  void test_performance_concurrent_access() {
    ...
    for (int t = 0; t < num_threads; ++t) {
      futures.push_back(
          std::async(std::launch::async, [&r, requests_per_thread]() {
  ...
  void test_cache_performance() {
    ...
    api_group->get("/users/:id/posts/:postId",
                   std::make_shared<TestHandler>("cached_handler"));
    ...
    int result = r.find_route(HttpMethod::GET, "/api/users/123/posts/456",
                              found_handler, params, query_params);
    ...
    for (int i = 0; i < 1000; ++i) {
      result = r.find_route(HttpMethod::GET, "/api/users/123/posts/456",
                            found_handler, params, query_params);
      assert(result == 0);
    }
    double cached_lookups = timer.elapsed_ms();

    std::cout << "  First lookup (no cache): " << first_lookup << "ms"
              << std::endl;
    std::cout << "  Average cached lookup: " << cached_lookups / 1000.0 << "ms"
              << std::endl;
    std::cout << "  Cache speedup: "
              << (first_lookup / (cached_lookups / 1000.0)) << "x" << std::endl;
  ...
  void test_group_static_routes_comprehensive() {
    ...
    std::vector<std::tuple<std::string, std::string, std::string>>
        group_routes = {{"/users", "get_users", "api_v1"},
    ...
    for (const auto &[path, handler_name, group_name] : group_routes) {
      ...
      int result = r.find_route(HttpMethod::GET, full_path, found_handler,
                                params, query_params);
      assert(result == 0);
      assert(found_handler != nullptr);
  ...
  void test_group_parameterized_routes_comprehensive() {
    ...
    v1_group->get("/posts/:slug",
                  std::make_shared<TestHandler>("v1_post_by_slug"));

    v2_group->get("/users/:userId",
                  std::make_shared<TestHandler>("v2_user_by_id"));
    v2_group->get("/articles/:articleId",
                  std::make_shared<TestHandler>("v2_article_by_id"));
  ...
  void test_group_wildcard_routes_comprehensive() {
    ...
    cdn_group->get("/:version/files/*",
                   std::make_shared<TestHandler>("versioned_files"));
    ...
    std::vector<std::tuple<std::string, std::string,
        ...
        test_cases = {
            ...
            {"/static/assets/js/app.min.js",
             "js_assets",
             {{"*", "app.min.js"}}},
            {"/static/assets/js/vendor/jquery.js",
             "js_assets",
             {{"*", "vendor/jquery.js"}}},
            {"/static/assets/css/main.css", "css_assets", {{"*", "main.css"}}},
            {"/static/assets/css/themes/dark.css",
             "css_assets",
             {{"*", "themes/dark.css"}}},
  ...
  void test_group_middleware_comprehensive() {
    ...
    std::vector<std::tuple<std::string, std::string>> test_cases = {
        ...
        {"/api/v1/admin/users", "admin_users_admin_perm_v1_version_api_auth"}
        // All middlewares
    };
  ...
  void test_group_large_scale_performance() {
    ...
    for (int g = 0; g < num_groups; ++g) {
      ...
      group->use([g](std::shared_ptr<TestHandler> &handler) {
        auto original = handler;
        handler = std::make_shared<TestHandler>(original->name + "_g" +
                                                std::to_string(g));
      });
      ...
      for (int r = 0; r < routes_per_group; ++r) {
        ...
        group->get(path, std::make_shared<TestHandler>("handler_" +
                                                       std::to_string(g) + "_" +
                                                       std::to_string(r)));
      }
    ...
    std::cout << "  Registered " << total_routes << " routes in " << num_groups
              << " groups in " << registration_time << "ms" << std::endl;
    ...
    for (int i = 0; i < lookup_iterations; ++i) {
      ...
      std::string path = "/group" + std::to_string(group_id) + "/route" +
    ...
    std::cout << "  Performed " << lookup_iterations << " lookups in "
...
// External function to run router group tests
int run_router_group_tests() {
  try {
    RouterGroupTest test_suite;
    test_suite.run_all_tests();
    return 0;
  } catch (const std::exception &e) {
    std::cerr << "Router group test failed with exception: " << e.what()
              << std::endl;
    return 1;
  } catch (...) {
    std::cerr << "Router group test failed with unknown exception" << std::endl;
    return 1;
  }
}
